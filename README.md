...
- Automated pipeline: every push to `master` builds Docker images, runs the full test suite, and deploys to production without manual steps
- Smart caching at two levels — static files (JS, CSS, images) served from Cloudflare's global network; course data cached in memory on the server
