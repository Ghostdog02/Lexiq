...
- **Named types over anonymous types in LINQ**: Replace `new { }` anonymous projections with named `private sealed class` types when the same shape is used in multiple methods (e.g. `UserProgressJoin`, `UserGroupKey` in `LeaderboardService`). **Critical EF Core constraint**: intermediate pipeline steps (`Join`, `GroupBy`) require **object initializer syntax** (`new Type { Prop = val }` → `MemberInitExpression`) — positional constructor syntax (`new Record(val)` → `NewExpression` without `Members`) fails because EF Core cannot map constructor args to SQL columns. Use `sealed class` with `{ get; init; }` properties, not positional `record` types, for Join/GroupBy projections. Terminal `.Select()` before `.ToListAsync()` can use positional records (materialises client-side).
